...
import { Search, ChevronDown, Calendar, User, Filter, Loader2, AlertCircle, MessageSquare, CheckCircle, ChevronLeft, ChevronRight, Package } from 'lucide-react';
...
const AdminOrders = ({ initialOrders, onUpdate }) => {
  ...
  const sortedOrders = useMemo(() => {
    ...
    // Sort logic: 
    // 1. Firebase orders (isFirebase: true) come first
    // 2. Then sort by date (newest first) within those groups
  ...
  return (
    <div className="space-y-6 animate-fade-in px-2 sm:px-0 pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-900/50 p-4 rounded-2xl border border-slate-800">
        <div>
            <h2 className="text-2xl font-bold text-white tracking-tight flex items-center gap-2"><Package className="text-violet-500"/> Order Management</h2>
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            ...
            <tbody className="divide-y divide-slate-800/50">
              {currentItems.length > 0 ? (
                currentItems.map(o => (
                  <tr key={o.order_id} className={`hover:bg-slate-800/30 transition-all group ${o.order_status === 'Return Requested' ? 'bg-orange-500/5' : ''}`}>
                    <td className="p-5">
                    ...
                    <td className="p-5">
                      <div className="flex flex-col items-start gap-2">
                          ...
                          {o.order_status === 'Return Requested' && o.returnReason && (
                              <div className="flex items-start gap-2 mt-1 p-2 rounded-lg bg-orange-500/10 border border-orange-500/20 max-w-[220px] animate-fade-in-up">
        ...
        {currentItems.length > 0 ? (
            currentItems.map(o => (
                <div key={o.order_id} className={`bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg relative overflow-hidden ${o.order_status === 'Return Requested' ? 'border-orange-500/30' : ''}`}>
                    {o.order_status === 'Return Requested' && <div className="absolute top-0 right-0 w-16 h-16 bg-orange-500/10 rounded-bl-full -mr-8 -mt-8 z-0"></div>}
