...
import { 
  ...
  ArcElement, Title, Tooltip, Legend, Filler // ✅ Added Filler here
} from 'chart.js';
import { 
  LayoutDashboard, ShoppingBag, Users, Package, Settings, FileText, 
  BarChart2, Search, Menu, X, LogOut, User, MapPin, Mail, Camera, Loader, Bell // ✅ Ensure X is imported here
} from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  return (
    <div className="flex flex-col h-screen w-full font-sans text-slate-200 overflow-hidden bg-[#0f172a] selection:bg-violet-500/30">
      ...
      {/* NAVBAR */}
      <nav className="h-20 bg-black/20 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-6 lg:px-10 z-50 shrink-0">
      ...
      {/* CONTENT */}
      <main className="flex-1 overflow-y-auto p-6 lg:p-10 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent relative z-10">
        {loadingData ? (
          ...
          <div className="max-w-7xl mx-auto">
            {activeSection === 'dashboard' && (
              <div className="space-y-8 animate-fade-in-up">
                <div className="mb-8">
                  <h1 className="text-3xl font-bold text-white">Welcome back, {adminProfile.firstName}</h1>
                    ...
                    <div key={idx} className="relative overflow-hidden bg-white/5 border border-white/5 p-6 rounded-3xl group hover:border-white/10 transition-all duration-300 hover:-translate-y-1 shadow-xl">
                      ...
                      <div className="relative z-10 flex justify-between items-start">
                        <div>
                          ...
                          <h3 className={`text-3xl font-bold ${stat.color} drop-shadow-sm`}>{stat.value}</h3>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-2 bg-white/5 border border-white/5 rounded-3xl p-8 backdrop-blur-sm relative overflow-hidden shadow-xl">
                  ...
                  <div className="bg-white/5 border border-white/5 rounded-3xl p-8 backdrop-blur-sm relative shadow-xl">
                    <h3 className="text-lg font-bold text-white mb-6">Order Status</h3>
                    <div className="h-64 flex justify-center">
                      <Pie data={{ 
                        labels: Object.keys(statusCount), 
                        datasets: [{ data: Object.values(statusCount), backgroundColor: ['#fbbf24', '#3b82f6', '#10b981', '#f43f5e'], borderWidth: 0 }] 
                      }} 
                      options={{ plugins: { legend: { position: 'bottom', labels: { color: '#94a3b8', usePointStyle: true, padding: 20 } } } }} />
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-fade-in">
          <div className="bg-[#0f172a] border border-white/10 rounded-3xl w-full max-w-xl shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 flex justify-end relative z-10">
              {/* ✅ X is now correctly imported and used here */}
