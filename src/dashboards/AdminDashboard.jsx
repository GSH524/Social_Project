...
import { 
  ...
  BarChart2, Menu, X, User, Camera, Loader
} from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      // 2. ORDERS (Merged: Static Demo + Firebase Live)
      
      // A. Calculate Static "Demo" Orders
      ...
      // B. Fetch LIVE Orders from 'OrderItems'
      ...
      const firebaseOrders = firebaseSnap.docs.map(d => {
        ...
        return {
          ...
          customer_id: data.userId || data.email || 'Guest', // Added Customer ID
  ...
  const totalRevenue = orders.filter(o => o.order_status !== "Cancelled" && o.order_status !== "Returned").reduce((sum, o) => sum + (Number(o.order_total_amount) || 0), 0);
  ...
  return (
    // 🔹 UPDATE: Removed 'fixed inset-0 z-[999]'
    // 🔹 UPDATE: Added 'h-[calc(100vh-80px)]' to fit perfectly below the Main Navbar (assuming 80px height)
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-2 bg-white/5 border border-white/5 rounded-3xl p-6 lg:p-8 backdrop-blur-sm relative overflow-hidden shadow-xl">
                    ...
                    <div className="h-60 lg:h-72 w-full">
                      <Line data={{ labels: ['J','F','M','A','M','J'], datasets: [{ label: 'Revenue', data: [12000,19000,15000,25000,22000,30000], borderColor: '#8b5cf6', backgroundColor: 'rgba(139, 92, 246, 0.1)', fill: true, tension: 0.4, pointBackgroundColor: '#8b5cf6' }] }} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { grid: { color: 'rgba(255,255,255,0.05)' } }, x: { grid: { display: false } } } }} />
                    </div>
                  </div>
                  <div className="bg-white/5 border border-white/5 rounded-3xl p-6 lg:p-8 backdrop-blur-sm relative shadow-xl">
                    ...
                    <div className="h-64 flex justify-center">
                      <Pie data={{ labels: Object.keys(statusCount), datasets: [{ data: Object.values(statusCount), backgroundColor: ['#fbbf24', '#3b82f6', '#10b981', '#f43f5e'], borderWidth: 0 }] }} options={{ plugins: { legend: { position: 'bottom', labels: { color: '#94a3b8', usePointStyle: true, padding: 20 } } } }} />
                    </div>
                  </div>
                </div>
              </div>
